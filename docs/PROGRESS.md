...
- Attention Inbox upgrade: `I` now prefers a cache-first, `gh search prs` backed Inbox when the adapter supports it, and `vivecaka inbox` launches straight into the global queue. Sources include review requests, assignments, current repo, favorites, owned repos, and CI-failing overlays. Ranking profiles are configurable via `[inbox] rank_profile`. Rows use symbol-forward foreground styling with stable staged updates, no selected-row background fill, and `f` explicitly focuses a repo when desired.
